...
- **Never ship a control that lies.** Features that are not built yet are rendered disabled with a "Coming soon!" tooltip and a BACKLOG.md entry — not as live controls that silently do nothing. The `soon_button()` macro renders them; today it stands in for Install next to a dependency with no unattended install command (pango on Windows), with the manual command shown beside it. Cancel was held to it longest: killing the Python parent while ffmpeg carried on would have been exactly the kind of lie this forbids.
- **Don't claim what isn't known.** Output files are listed only when the script reported them or they demonstrably appeared during the run, never guessed from a naming pattern. The progress bar is determinate only while a script actually reports a fraction and indeterminate otherwise; it never animates a made-up percentage. The same rule puts a model's download size on the form only when the server could learn it.
