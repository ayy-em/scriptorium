...
def _stub_duration_probe():
    ...
    with patch("scripts.av.video_crop.probe_duration_or_none", return_value=None):
        yield
...
def test_crop_builds_correct_ffmpeg_command(tmp_path):
    ...
    with (
        ...
        patch("scripts.av.video_crop.run_ffmpeg_with_progress") as mock_ff,
    ):
        crop(Path("in.mp4"), out, top=100, right=200, bottom=100, left=200)
    mock_ff.assert_called_once()
    assert mock_ff.call_args[0][0] == ["-i", "in.mp4", "-vf", "crop=1520:880:200:100", str(out)]
